...
Patch3: cflags.patch
Patch4: rsyncable-always.patch
Patch5: always-rsync.patch
...
%patch -P 5 -p1
pushd ..
cp -a zstd-1.5.7 build32
popd
pushd ..
cp -a zstd-1.5.7 buildavx2
popd
pushd ..
cp -a zstd-1.5.7 buildavx512
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1740070546
...
pushd build/meson
export GOAMD64=v2
meson --libdir=lib64 --prefix=/usr --buildtype=plain -Ddefault_library=both \
-Dbin_programs=false  builddir
ninja -v -C builddir
GOAMD64=v3
CFLAGS="$CFLAGS -march=x86-64-v3 -Wl,-z,x86-64-v3 " CXXFLAGS="$CXXFLAGS -march=x86-64-v3 -Wl,-z,x86-64-v3 " LDFLAGS="$LDFLAGS -march=x86-64-v3 " meson --libdir=lib64 --prefix=/usr --buildtype=plain -Ddefault_library=both \
-Dbin_programs=false  builddiravx2
ninja -v -C builddiravx2
GOAMD64=v4
CFLAGS="$CFLAGS -march=x86-64-v4 -mprefer-vector-width=512 -Wl,-z,x86-64-v4 " CXXFLAGS="$CXXFLAGS -march=x86-64-v4 -mprefer-vector-width=512 -Wl,-z,x86-64-v4 " LDFLAGS="$LDFLAGS -march=x86-64-v4 " meson --libdir=lib64 --prefix=/usr --buildtype=plain -Ddefault_library=both \
...
%files lib
%defattr(-,root,root,-)
/V3/usr/lib64/libzstd.so.1.5.7
/V4/usr/lib64/libzstd.so.1.5.7
